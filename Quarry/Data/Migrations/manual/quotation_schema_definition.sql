/*
  Quotation module schema for SQL Server
  - Tables: [dbo].[Quotations], [dbo].[QuotationItems]
  - Matches domain models: [Quotation](Models/Domain/Quotation.cs), [QuotationItem](Models/Domain/QuotationItem.cs)
  - Includes UQ on QuotationNumber, FKs to Customers and (optionally) Materials
...
BEGIN
    ...
    CREATE UNIQUE INDEX [UX_Quotations_QuotationNumber]
        ON [dbo].[Quotations]([QuotationNumber]);

    -- FK to Customers (required)
    ...
    ELSE
    BEGIN
        PRINT 'WARNING: Customers table not found. Add FK [FK_Quotations_Customers_CustomerId] later.';
    END
END
GO

-- ==============================
-- QuotationItems (Detail Lines)
-- ==============================
...
BEGIN
    ...
    -- For fast retrieval by quotation
    CREATE INDEX [IX_QuotationItems_QuotationId]
        ON [dbo].[QuotationItems]([QuotationId]);

    -- Required FK to Quotations (cascade delete)
    ...
    -- Optional FK to Materials (guarded to avoid error on DBs without Materials)
    ...
    ELSE
    BEGIN
        PRINT 'INFO: Materials table not found. Skipping FK [FK_QuotationItems_Materials_MaterialId].';
    END
END
GO
...
-- ======================
-- SELECT TOP 0 * FROM [dbo].[Quotations];
-- SELECT TOP 0 * FROM [dbo].[QuotationItems];